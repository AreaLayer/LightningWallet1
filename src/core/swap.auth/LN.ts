...
const loginMnemonic = (mnemonic, walletNumber=0, path, app) => {
  const network = (
    app.isMainNet()
      ? app.env.lightning.networks.mainnet
      : app.env.lightning.networks.testnet
  )

  const wallet = mnemonicUtils.getLightningWallet(network, mnemonic, walletNumber, path)
...
const login = (_privateKey, app) => {
  ...
  const network = (
    app.isMainNet()
      ? app.env.lightning.networks.mainnet
      : app.env.lightning.networks.testet
  )
